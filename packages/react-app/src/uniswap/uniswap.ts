...
import uni from '@uniswap/sdk'
import { ethers } from 'ethers'
import math from 'mathjs'
import { Router__factory } from '../types/ethers-contracts/factories/Router__factory';
import { Token__factory } from '../types/ethers-contracts/factories/Token__factory';
...
export default class Uniswap {
  ...
  slippage: any;
  // slippage: Fraction | math.MathArray | math.Matrix;
  pairsCacheTime: number;
  gasLimit: number;
  expireTokenPairUpdate: number;
  allowedSlippage: uni.Percent;
  zeroReserveCheckInterval: number;
  zeroReservePairs: any;
  tokenList:any;
  pairs:any;
  tokenSwapList:any;
  cachedRoutes:any;
  ...
  constructor (
    ...
    this.router = router;
    this.slippage = math.fraction(process.env.UNISWAP_ALLOWED_SLIPPAGE || 5)
    this.allowedSlippage = new uni.Percent(this.slippage.d, (this.slippage.d * 100))
    this.pairsCacheTime = 100
    this.gasLimit = GAS_LIMIT
    this.expireTokenPairUpdate = UPDATE_PERIOD
    this.zeroReserveCheckInterval = 100 // process.env.UNISWAP_NO_RESERVE_CHECK_INTERVAL
  ...
  async swapExactIn (wallet, trade, tokenAddress, gasPrice) {
    ...
    const contract = Router__factory.connect(exchange.exchangeRouterAddress, provider);
